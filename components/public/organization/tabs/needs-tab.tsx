...
const URGENCY_MAP: Record<string, { label: string; bg: string; text: string; dot: string }> = {
  alta: { label: "Urgencia Alta", bg: "bg-red-50 dark:bg-red-950/40", text: "text-red-600 dark:text-red-400", dot: "bg-red-600 dark:bg-red-400" },
  media: { label: "Urgencia Media", bg: "bg-amber-50 dark:bg-amber-950/40", text: "text-amber-600 dark:text-amber-400", dot: "bg-amber-500 dark:bg-amber-400" },
    ...
    <div className="py-2 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="mb-10">
        ...
        <p className="text-muted-foreground mt-2 font-medium text-sm max-w-2xl">
          Descubre cómo puedes apoyar a esta organización donando materiales, tu tiempo o recursos económicos.
        </p>
      </div>
      ...
      {activeNeeds.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {activeNeeds.map((need, idx) => {
            ...
            return (
              <Card key={idx} className="rounded-2xl border-border/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] transition-all duration-300 hover:-translate-y-1 overflow-hidden h-full flex flex-col group py-0 bg-background/50">
                <CardContent className="p-6 flex-1 flex flex-col relative pt-8">
                  ...
                  <div className="flex justify-between items-start mb-4 gap-4">
                    ...
                    <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-full ${urgency.bg} ${urgency.text} shrink-0`}>
                      <div className={`w-1.5 h-1.5 rounded-full ${urgency.dot}`} />
                      <span className="text-[10px] font-bold uppercase tracking-widest">{urgency.label}</span>
                    </div>
                  </div>
